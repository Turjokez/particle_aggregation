...
function analyze_model_eofs(title_prefix, t, Y, cfg, eps_run)
    ...
    % Flip sign of Mode 1 so it is mostly one sign (attenuation mode)
    ...
    % Optional: flip Mode 2 so large sizes are positive
    mode2 = reshape(coeffs(:,2), Ns, Nz).';
    if mean(mode2(:,end)) < 0  % look at largest sizes
        coeffs(:,2) = -coeffs(:,2);
        scores(:,2) = -scores(:,2);
        mode2       = -mode2;
    end
    
    % ---------- FIGURE 1: EOF maps + forcing / PC2 -----------------------
    figure('Name',[title_prefix ' EOF Analysis'],'Color','w', ...
           'Position',[50 50 1400 900]);
    
    subplot(2, 2, 1); 
    semilogy(t, max(eps_run,1e-13), 'k-'); 
    ylabel('\epsilon (W kg^{-1})'); 
    title('Turbulence Forcing'); 
    axis tight; grid on;
    
    subplot(2, 2, 2); 
    plot(t, normalize(scores(:,2),'range'), 'r-'); 
    ylabel('PC2 (normalized)'); 
    title(sprintf('PC2 (Seesaw, %.1f%% var)', explained(2))); 
    axis tight; grid on;
...
function plot_psd_slice_subplot(ax_handle, t_days, depths, D_mm, PSD_3d, ...
    ...
    % one "fence" per day; flip so earliest days are closest to viewer
    ...
    % vertical colourbar on the side (not huge)
    ...
    view([-60 05]);             % earliest days in front

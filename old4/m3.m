function master_simulation_runner
    ...
    % Version: Atlantic forcing, Square-wave test, EOF + 3D slice plots.
    ...
    fprintf('  [1] Real Atlantic Data (Relative Flux + Size Classes)\n');
    fprintf('  [2] Square Wave Test (Idealized Lab Test)\n');
    fprintf('  [3] EOF Analysis (Atlantic Validation + 3D Plots)\n');
...
function make_comprehensive_plots(result, sim_grid_obj, cfg, eps_run, ...
    ...
    % radii, velocities, volumes, diameters
    ...
    D_um    = 2 * r_cm * 1e4;                  % diameters in micron
    ...
    % Steady-state flux (background epsilon, low eps days)
    ...
    % ---- Size classes in µm (Adrian suggestion) -------------------------
    % small  : D < 200  µm
    % medium : 200 ≤ D < 1000 µm
    % large  : D ≥ 1000 µm (1 mm)
    D_small_max  = 200; 
    D_medium_max = 1000; 
...
function analyze_model_eofs(title_prefix, t, Y, cfg, eps_run)
    ...
    fig1 = figure('Name',[title_prefix ' EOF Analysis'],'Color','w', ...
    ...
    % colour limits from 2nd–98th percentiles
...
function plot_psd_slice_subplot(ax_handle, t_days, depths, D_mm, PSD_3d, ...
    ...
    % one "fence" per day; earliest days at the front (like the EXPORTS plot)
    ...
    % log D axis with ticks at 10^-1, 10^0, 10^1 mm
    set(gca,'YTick', log10([0.1 1 10]), ...
            'YTickLabel', {'10^{-1}','10^{0}','10^{1}'}, ...
            'TickLabelInterpreter','tex');

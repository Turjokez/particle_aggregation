% =========================
% FILE: BetaAssembler.m
% =========================
classdef BetaAssembler < handle
    %BETAASSEMBLER Computes sectionally integrated coagulation kernel matrices
    %
    % NOTE (NEW-2026-01-12):
    % This assembler builds b1..b5 using the historical finite-bin sectional scheme.
    % In a finite grid, collisions can generate products beyond the largest bin.
    % That "overflow" must be handled by a closure.
    %
    % We enforce that closure in CoagulationRHS (moment closure into last bin),
    % so we keep BetaAssembler unchanged for stability and reproducibility.
    %
    % NEW-2026-01-13 (BETA ORIENTATION NOTE):
    % Your conservation tests show the row-form evaluation expects:
    %   b2: nr*M
    %   b3: nr*M'
    %   b4: nr*M
    %   b5: nr*M
    %   b1: nr*M'
    % We implement that safely in CoagulationRHS (recommended).
    % If you ever want to store pre-transposed matrices here, keep old ones too.
    %
    % IMPORTANT: We do NOT delete anything in this file.
    properties
    ...
    methods
        ...
        function betas = combineAndScale(obj, b_brown, b_shear, b_ds)
            ...
            if ~isempty(b_brown_scaled.b1) || ~isempty(b_shear_scaled.b1) || ~isempty(b_ds_scaled.b1)
                ...
                % -------------------------------------------------------------
                % NEW: keep b25 ONLY as a diagnostic "net" matrix.
                % NOTE: Do NOT use b25 inside RHS time-stepping.
                betas.b25 = betas.b2 - betas.b3 - betas.b4 - betas.b5;

                % NEW: quick sanity check (optional)
                if any(~isfinite(betas.b25(:)))
                    warning('BetaAssembler: b25 has non-finite values.');
                end

                % -------------------------------------------------------------
                % NEW-2026-01-13 OPTIONAL (kept OFF by default):
                % If you ever want to store "RHS-ready" transposed versions here,
                % do it ONLY as additional fields, never overwriting originals.
                % Example (do not activate unless you want it):
                %
                % if isprop(obj.config,'beta_store_rhs_ready') && obj.config.beta_store_rhs_ready
                %     betas.b1_rhs = betas.b1.';   % so RHS can do nr*b1_rhs
                %     betas.b3_rhs = betas.b3.';   % so RHS can do nr*b3_rhs
                % end
                % -------------------------------------------------------------
            else
                betas = BetaMatrices();
        ...
        function betas_scaled = scaleBetas(obj, betas, scale_factor)
            ...
            % -------------------------------------------------------------
            % NEW (safe): if b25 exists in the input, scale it; otherwise compute it
            if isprop(betas,'b25') && ~isempty(betas.b25)
                betas_scaled.b25 = betas.b25 * scale_factor;
            else
                betas_scaled.b25 = (betas.b2 - betas.b3 - betas.b4 - betas.b5) * scale_factor;
            end
        end
    end
    ...
    methods (Access = private)
        function betas = computeBetaMatrices(obj)
            ...
            % NEW: use grid-provided bounds (allows special handling at top bin)
            mlo = obj.grid.v_lower;
            mup = obj.grid.v_upper;
            ...
            for jcol = 1:(n_sections - 1)
                for irow = (jcol + 1):n_sections
                    ...
                    % NOTE: quadl cannot integrate to Inf robustly for our kernel forms.
                    % If the last bin is open-ended (Inf), we cap for quadrature at the
                    % historical value (2*mlo) to keep behavior stable.
                    if ~isfinite(mj_up), mj_up = 2.0 * mj_lo; end
                    if ~isfinite(mi_up), mi_up = 2.0 * mi_lo; end
        ...
        % ---- integration functions unchanged (kept exactly) ----

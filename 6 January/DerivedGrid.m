...
classdef DerivedGrid < handle
    ...
    properties
        ...
        % NEW-2026-01-12:
        % Finite (non-Inf) upper bounds copy for diagnostics/weights
        % so sqrt(v_lower .* v_upper_closed) never becomes Inf.
        v_upper_closed; % NEW-2026-01-12
    ...
    methods
        ...
        function computeDerivedValues(obj, config)
            ...
            % NEW: keep standard doubling, but make last bin "open-ended"
            % NOTE:
            %   This helps avoid top-bin truncation/overflow issues in diagnostics
            %   and in any code that uses v_upper for bin logic.
            obj.v_upper = 2.0 * obj.v_lower;
            obj.v_upper(end) = Inf;

            % NEW-2026-01-12:
            % Keep a finite "closed" copy for any diagnostics/weights.
            % This prevents Inf in sqrt(v_lower .* v_upper) based weights.
            obj.v_upper_closed = 2.0 * obj.v_lower;

% ==============================================================
% FILE: KernelLibrary.m
% ==============================================================
% ASCII-safe version (prevents "Invalid text character" issues)
% ==============================================================
classdef KernelLibrary < handle
    ...
    methods (Static)
        ...
        function b = curvilinearDS(r, rcons, param)
            ...
            % r     = 2xN vector of particle radii [cm]
            % rcons = 2xN vector of conserved volume radii [cm]
            % param = struct including r_to_rg

            r_small = min(r) * param.r_to_rg;           % 1xN
            rcons_3 = rcons .* rcons .* rcons;          % 2xN
        ...
        function b = curvilinearShear(r, rcons, param)
            ...
            % r     = 2xN vector of particle radii [cm]
            % rcons = dummy variable
            % param = struct including r_to_rg

            %#ok<INUSD>  % rcons unused
            rg = (r(1,:) + r(2,:)) * param.r_to_rg;     % 1xN
        ...
        function kernel = getKernel(kernelName)
            ...
            switch kernelName
                ...
                otherwise
                    error('KernelLibrary:UnknownKernel', 'Unknown kernel: %s', kernelName);
            end
        end

    ...
    methods (Static)
        ...
        function output_data = spectraAndFluxes(t, Y, grid, config)
            ...
            if isprop(config, 'use_column') && config.use_column
                ...
                %
                % IMPORTANT FIX:
                % - Default should be "vbin" for number-like state so export is
                %   consistent with biovolume inventory space.
                % ======================================================
                weight_mode = "vbin";
                try
                ...
                % ======================================================
                % Step 2 (AUTO WEIGHT CHOICE): if state is already biovolume,
                % force weights to "ones" so you don't double-weight by vbin.
                % ======================================================
                try
                    if isprop(config,'state_is_biovolume') && config.state_is_biovolume
                        weight_mode = "ones";
                    end
                catch
                end
                ...
                % keep a legacy copy so we can see the mismatch later
                % ------------------------------------------------------
                column_total_inventory_cm3cm2_legacy = nan(n_times,1);
                ...
                for it = 1:n_times
                    ...
                    % ------------------------------------------------------
                    % NEW (ADDED, DOES NOT DELETE ANYTHING):
                    % TRUSTED STATE-BASED INVENTORY (RAW Y), cm3/cm2
                    % - fixes the mismatch you observed (M_out vs M_state)
                    % - we keep legacy value in column_total_inventory_cm3cm2_legacy
                    % ------------------------------------------------------
                    column_total_inventory_cm3cm2_legacy(it) = column_total_inventory_cm3cm2(it);

                    % raw state (Ns x Nz), converted to per cm^3 if needed
                    Yraw = reshape(Y(it,:).', [Ns Nz]) * N_to_cm3;   % Ns x Nz
                    M_state_cm3cm2 = sum(Yraw(:)) * dz_cm;          % cm3/cm2  (since dz_cm is per layer)

                    % overwrite the "official" cm^2 inventory with the trusted value
                    column_total_inventory_cm3cm2(it) = M_state_cm3cm2;

                    % ======================================================
                    % Step 3 (FIX N_bottom / N_surface):
                    % overwrite snapshots using TRUSTED STATE (Yraw),
                    % so debugging export uses exactly the same state as inventory.
                    % ======================================================
                    ysurf_trusted = Yraw(:, 1).';      % 1 x Ns
                    ybot_trusted  = Yraw(:, end).';    % 1 x Ns
                    N_surface(it, :) = ysurf_trusted;
                    N_bottom(it, :)  = ybot_trusted;

                    % also update ysurf/ybot used later in flux and spectra
                    ysurf = ysurf_trusted;
                    ybot  = ybot_trusted;

                    % optional one-time print
                    if it == 1
                        fprintf('[INV CHECK] cm3/cm2 legacy=%g | state=%g | diff=%g\n', ...
                            column_total_inventory_cm3cm2_legacy(it), column_total_inventory_cm3cm2(it), ...
                            column_total_inventory_cm3cm2_legacy(it) - column_total_inventory_cm3cm2(it));
                    end
                    ...
                    % ---- legacy spectra on bottom layer (uses ybot per cm^3 now) ----
                ...
                % ------------------------------------------------------
                % NEW (ADDED): keep legacy cm^2 inventory for debugging
                % ------------------------------------------------------
                output_data.column_total_inventory_cm3cm2_legacy = column_total_inventory_cm3cm2_legacy;
                ...
                % Also provide compatibility "best" totals:
                %   - Flux best: weighted (export_weight controls it)
                %   - Mass best: UNweighted, state-based (so it matches inventory from Y)
                output_data.bottom_total_flux_best = output_data.bottom_total_flux_weighted;
                output_data.total_flux_best        = output_data.bottom_total_flux_weighted;
                output_data.total_mass_best        = column_total_inventory_cm3m2;

                % NEW-2026-01-23: "best" totals in cm^2 space 
                ...
                output_data.total_mass_best_cm3cm2         = column_total_inventory_cm3cm2;
                ...
                try
                    ...
                    diag.M_cm3cm2_legacy = column_total_inventory_cm3cm2_legacy;

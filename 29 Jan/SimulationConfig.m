classdef SimulationConfig < handle
    ...
    properties
        ...
        % ==========================================================
        % NEW-2026-01-23: state meaning + units (IMPORTANT)
        % ==========================================================
        state_units = "cm3";          % "cm3" or "m3"
        state_is_biovolume = false;   % true if Y already represents biovolume concentration
    ...
    methods
        ...
        function validate(obj)
            ...
            % ==========================================================
            % NEW: validate state_units
            % ==========================================================
            if isprop(obj,'state_units') && ~isempty(obj.state_units)
                su = lower(string(obj.state_units));
                assert(any(su == ["cm3","m3","per_m3"]), 'state_units must be "cm3" or "m3".');
            end
            ...
            % ==========================================================
            % IMPORTANT: if state is biovolume, force export_weight="ones"
            % (THIS MUST BE INSIDE validate())
            % ==========================================================
            if isprop(obj,'state_is_biovolume') && ~isempty(obj.state_is_biovolume) && logical(obj.state_is_biovolume)
                if lower(string(obj.export_weight)) ~= "ones"
                    warning('state_is_biovolume=true -> forcing export_weight="ones".');
                    obj.export_weight = "ones";
                end
            end
        end

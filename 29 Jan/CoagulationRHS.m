classdef CoagulationRHS < handle
    ...
    % If cfg.enforce_coag_bv_closure == true, enforce closure:
    %   - If state_is_biovolume: sum(dv_coag)=0 (biovolume conservation)
    %   - Else: sum(Vbin .* dv_coag)=0 (number-state biovolume conservation)
    ...
    methods
        ...
        function dvk = applyCoagBvClosure(obj, dvk)
            ...
            if is_state_bv
                % biovolume-state: conserve sum(dvk)=0
                leak = sum(dvk);
                if ~isfinite(leak) || leak == 0
                    return;
                end
                dvk(end) = dvk(end) - leak;
                return;
            end

            % number-like state: conserve sum(V .* dvk)=0
            V = [];
            try
        ...
        % ==========================================================
        % PP source helper (robust: state_is_biovolume OR vbin OR state/day)
        % ==========================================================
        function dv_pp = getPPSourceVector(obj, v)
            ...
            % pp_rate (we will interpret it robustly below)
            pp_rate = 0.0;
            try
            ...
            if isprop(obj.config,'use_column') && obj.config.use_column
                ...
                if is_state_bv
                    % State is already biovolume. Interpret pp_rate as areal biovolume input [cm^3/cm^2/day]
                    % Convert to layer tendency by dividing by layer thickness.
                    dv_pp(idx) = dv_pp(idx) + pp_rate / dz_cm;

                elseif is_vbin
                    % State is NOT biovolume (typically number). Interpret pp_rate as areal biovolume input [cm^3/cm^2/day]
                    % Convert using bin volume and dz.
                    if isempty(obj.grid)
                    ...
                    dv_pp(idx) = dv_pp(idx) + pp_rate / (wbin(pp_bin) * dz_cm);

                else
                    % Otherwise pp_rate is already in state units/day
                ...
                % 0-D
                if is_state_bv
                    error('PP source: state_is_biovolume=1 but 0-D injection needs a clear volume convention. Use column mode or define 0-D conversion.');
                end
                if is_vbin
        ...
        function dvdt = evaluate(obj, t, v)
            ...
            if isprop(obj.config, 'use_column') && obj.config.use_column
                ...
                for k = 1:Nz
                    ...
                    % enforce closure (biovolume or V-weighted depending on state)
            ...
            % apply closure to the coag part in 0-D

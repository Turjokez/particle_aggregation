    ...
    methods (Static)
        ...
        function output_data = spectraAndFluxes(t, Y, grid, config)
            ...
            if isprop(config, 'use_column') && config.use_column
                ...
                % Inventory per area (cm^3/m^2) and export per area (cm^3/m^2/day)
                ...
                % ======================================================
                % NEW-2025-12-21: Create flux spectrum field expected by plots
                % We define a bottom-layer "flux spectrum" (per size bin) using:
                %   fluxspec_i_bottom ~ (mass spectrum) * w  (and include 1e4 for m^2)
                % This is mainly for plotting shape/consistency.
                % ======================================================
                fluxspec_i_bottom = (masspec_i_bottom .* set_vel_cmday(:)') * 1e4;  % [cm^3 m^-2 d^-1 per bin-width-ish]

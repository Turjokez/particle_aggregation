    ...
    methods (Static)
        ...
        function [gains, losses] = sectional(spec, operators)
            ...
            if use_column
                ...
                for it = 1:n_times
                    ...
                    % OLD (kept): rate-like loss (1/day * conc), NOT per-area flux
                    % sink_losses(it, :) = (sink_rate .* vbot)';

                    % OLD (kept): your earlier "w/dz" conversion idea
                    % sink_losses(it, :) = (sink_rate .* vbot * dz_cm * 1e4)';

                    % NEW-2025-12-21: compute bottom export from w (cm/day)
                    % F_s = 1e4 * (Nbot_s * w_s)  [cm^3 m^-2 d^-1] per section
                    if has_w
                        sink_losses(it, :) = (1e4 * (w_cmday .* vbot))';
                    else
                        % NEW-2025-12-21 (fallback): if w not available, keep old behavior
                        sink_losses(it, :) = (sink_rate .* vbot * dz_cm * 1e4)';
                    end
                end
                ...
                losses.settl  = sink_losses;   % bottom export (per-section), cm^3 m^-2 d^-1
        ...
        function [total_gains, total_losses] = total(spec, operators)
            ...
            if use_column
                ...
                for it = 1:n_times
                    ...
                    % NEW-2025-12-21: total bottom export flux from w (cm/day)
                    if has_w
                        total_sink_losses(it) = 1e4 * sum(w_cmday .* vbot);
                    else
                        total_sink_losses(it) = sum(sink_rate .* vbot) * dz_cm * 1e4;
                    end
        ...
        function closureCheckFromOutput(out)
            ...
            if isfield(out,'time')
                ...
                % OLD (kept): fallback if time missing
                % t = linspace(0, 30, numel(M))';
